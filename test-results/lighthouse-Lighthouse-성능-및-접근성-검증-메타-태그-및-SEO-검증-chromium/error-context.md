...
- Name: lighthouse.spec.ts >> Lighthouse 성능 및 접근성 검증 >> 메타 태그 및 SEO 검증
- Location: e2e\lighthouse.spec.ts:96:7
...
```ts
  17  | 
  18  |     console.log("Navigation Timing:", navigationTiming);
  19  | 
  20  |     // 로드 시간 검증 (3초 이내 권장)
  21  |     expect(navigationTiming.totalTime).toBeLessThan(5000);
  22  |   });
  23  | 
  24  |   test("Core Web Vitals 측정", async ({ page }) => {
  25  |     await page.goto("/", { waitUntil: "domcontentloaded" });
  26  |     await page.waitForTimeout(1000);
  27  | 
  28  |     // CLS (Cumulative Layout Shift) 측정
  29  |     const cls = await page.evaluate(() => {
  30  |       return new Promise<number>((resolve) => {
  31  |         let clsValue = 0;
  32  |         const observer = new PerformanceObserver((list) => {
  33  |           for (const entry of list.getEntries()) {
  34  |             if ((entry as any).hadRecentInput) continue;
  35  |             clsValue += (entry as any).value;
  36  |           }
  37  |         });
  38  |         observer.observe({ entryTypes: ["layout-shift"] });
  39  | 
  40  |         setTimeout(() => {
  ...
  45  |     });
  46  | 
  47  |     console.log("CLS:", cls);
  48  |     // CLS는 0.1 이하가 good
  49  |     expect(cls).toBeLessThan(0.25);
  50  |   });
  51  | 
